...
# #######################################################
# 
@testset "runtests_ClaSSE_tree_n7a_difftips.jl -- 3 apes, DEC+largeJ+largeMu, diff. tip states" begin
# 
#######################################################
...
#
# Run with:
# source("/GitHub/PhyBEARS.jl/R_examples/_compare_ClaSSE_calcs_n7a_difftips_compare2julia.R")
# Truth:
R_result_branch_lnL = -8.060856
R_result_total_LnLs1 = -11.880469
R_result_total_LnLs1t = -6.707179
R_result_sum_log_computed_likelihoods_at_each_node_x_lambda = -13.75783
#######################################################
...
res.likes_at_each_nodeIndex_branchTop[2] = [0.0, 1.0, 0.0]			# state 2 for tip #2
res.normlikes_at_each_nodeIndex_branchTop[2] = [0.0, 1.0, 0.0]	# state 2 for tip #2
...
# root=ROOT.OBS, root.p=NULL, condition.surv=FALSE
@test round(R_result_total_LnLs1; digits=2) == round(Julia_total_lnLs1; digits=2)

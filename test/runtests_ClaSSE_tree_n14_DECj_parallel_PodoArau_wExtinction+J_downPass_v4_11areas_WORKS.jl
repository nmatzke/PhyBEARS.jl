...
# Add multiple cores/workers to match
using Distributed
Distributed.nprocs()
...
include("/GitHub/PhyBEARS.jl/test/runtests_ClaSSE_tree_n14_DECj_parallel_PodoArau_wExtinction+J_downPass_v4_11areas.jl")
"""
...
geog_df = getranges_from_LagrangePHYLIP(lgdata_fn)
...
# Home desktop:
# This one has to pass p_Ds_v5 to each core, makes it very slow
# julia --startup-file=yes : Doesn't finish, needs multiple cores
# JULIA_NUM_THREADS=24 julia --startup-file=yes : Doesn't finish, needs multiple cores
# JULIA_NUM_THREADS=24 julia -p 23 --startup-file=yes
solver_options=inputs.solver_options; max_iterations=10^6; return_lnLs=true; printlevel=1
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7d!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^7, return_lnLs=true)
...
#######################################################
# v7c From PhyBEARS install:
#######################################################
using Hwloc, Distributed
include("/GitHub/PhyBEARS.jl/notes/remotecall_fetch_trial_v2.jl")
#@everywhere using PhyloBits.TreeTable
@everywhere p_Ds_v7 = $p_Ds_v5

# 40 seconds on home desktop; 10 seconds on work desktop

# Home desktop
# julia --startup-file=yes : Crash, needs multiple threads
# JULIA_NUM_THREADS=24 julia --startup-file=yes : 13 seconds
# JULIA_NUM_THREADS=24 julia -p 23 --startup-file=yes : 13 seconds
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7cc!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^4.5, return_lnLs=true)
@benchmark (total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7c!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^4.5, return_lnLs=true)
...
@everywhere function branchOp_ClaSSE_Ds_v7_retDs(u0, tspan)
	calc_start_time = Dates.now()
	tmp_threadID = Hwloc.getinfo()[:Core]
...
@everywhere solver_options = $inputs.solver_options
@everywhere p_Ds_v7 = $p_Ds_v5

max_iterations=10^5; return_lnLs=true
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7ee!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^4.5, return_lnLs=true, printlevel=0)
# JULIA_NUM_THREADS=24 julia -p 23 --startup-file=yes : 11 seconds
# (10.163, 1628, -2035.4940619062409, -26.126455695173323, -2061.620517601414, -1351.4755730003967, "manual")

@benchmark (total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7ee!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^4.5, return_lnLs=true, printlevel=0)



#######################################################
# WORKED UNTIL HERE, 2022-07-24
#######################################################

example_one_branch = """
u0 = deepcopy(res.likes_at_each_nodeIndex_branchTop[1,:][1])
tspan = (0.0, 1.0)
jobs = @spawnat :any branchOp_ClaSSE_Ds_v7_retDs(u0, tspan)
tasks = @async fetch(jobs)
istaskstarted(tasks)
istaskdone(tasks)
istaskdone(tasks)
istaskdone(tasks)
...
u0 = res.likes_at_each_nodeIndex_branchTop[1,:][1]
p_Ds_v7=p_Ds_v5
solver_options=inputs.solver_options
max_iterations=10^5.5
return_lnLs=true

include("/GitHub/PhyBEARS.jl/notes/remotecall_fetch_trial_v2.jl")
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7dd!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^6, return_lnLs=true)


current_nodeIndex = 1
u0 = res.likes_at_each_nodeIndex_branchTop[1,:][1]
tspan = (0.0, 10.0)
x = Distributed.remotecall(branchOp_ClaSSE_Ds_v7, 2, current_nodeIndex, res, u0=deepcopy(u0), tspan=tspan, p_Ds_v7=p_Ds_v5, solver_options=solver_options)

x = Distributed.@spawnat :any branchOp_ClaSSE_Ds_v7(current_nodeIndex, res, u0=deepcopy(u0), tspan=tspan, p_Ds_v7=p_Ds_v5, solver_options=solver_options)
y = @async fetch(x)
istaskstarted(y)
istaskdone(y)
istaskdone(y)
...
@test (Julia_sum_lq_nodes - R_sum_lq_nodes) < 0.02
...
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v6!(res; trdf=trdf, p_Ds_v5=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)

(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v7!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)

(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)
...
@everywhere p_Ds_v7 = $p_Ds_v5
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7c!(res; trdf=trdf, p_Ds_v7=p_Ds_v5, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)

pars = bmo.est[bmo.type .== "free"]
func = x -> func_to_optimize_v7c(x, parnames, inputs, p_Ds_v5; returnval="bgb_lnL", printlevel=1)
pars = [0.9, 0.9, 0.9]
func(pars)
...
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v5!(res; trdf=trdf, p_Ds_v5=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v6!(res; trdf=trdf, p_Ds_v5=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v7!(res; trdf=trdf, p_Ds_v7=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7o!(res; trdf=trdf, p_Ds_v7=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^6, return_lnLs=true)
...
#######################################################
pars = bmo.est[bmo.type .== "free"]
func = x -> func_to_optimize_parallel(x, parnames, inputs, p_Ds_v5; returnval="bgb_lnL", printlevel=1)
pars = [0.9, 0.9, 0.9]
func(pars)
function func2(pars, dummy_gradient!)
	return func(pars)
...
func = x -> func_to_optimize_parallel(x, parnames, inputs, p_Ds_v5; returnval="lnL")
...
(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v5!(inputs.res; trdf=trdf, p_Ds_v5=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)

(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v6!(inputs.res; trdf=trdf, p_Ds_v5=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)

(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_nonparallel_ClaSSE_v7!(inputs.res; trdf=trdf, p_Ds_v7=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)

(total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL) = iterative_downpass_parallel_ClaSSE_v7!(inputs.res; trdf=trdf, p_Ds_v7=p_Ds_v7, solver_options=inputs.solver_options, max_iterations=10^5, return_lnLs=true)

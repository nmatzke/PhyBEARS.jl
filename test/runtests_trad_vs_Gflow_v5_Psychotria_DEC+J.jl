...
include("/GitHub/PhyBEARS.jl/test/runtests_trad_vs_Gflow_v5_Psychotria_DEC+J.jl")
"""

@testset "runtests_trad_vs_Gflow_v5_Psychotria_DEC+J.jl" begin
...
@test mean(abs.(interp_from_Gmap(0.1, Gmap) .- Gflow_to_01_GMRES(0.1))) < 0.00001


Gflows_array_totals[:,:,2]
interp_from_Gmap(0.2, Gmap)
Gflow_to_01_GMRES(0.2)
@test mean(abs.(interp_from_Gmap(0.2, Gmap) .- Gflow_to_01_GMRES(0.2))) < 0.00001


Gflows_array_totals[:,:,3]
interp_from_Gmap(0.3, Gmap)
Gflow_to_01_GMRES(0.3)
@test mean(abs.(interp_from_Gmap(0.3, Gmap) .- Gflow_to_01_GMRES(0.3))) < 0.00001

Gflows_array_totals[:,:,50]
interp_from_Gmap(5.0, Gmap)
Gflow_to_01_GMRES(5.0)
@test mean(abs.(interp_from_Gmap(5.0, Gmap) .- Gflow_to_01_GMRES(5.0))) < 0.00001
...
@test round(Julia_sum_lq_nFv6, digits=1) == round(Julia_sum_lq_GFv6a, digits=1)
@test round(rootstates_lnL_nFv6, digits=2) == round(rootstates_lnL_GFv6a, digits=2)
@test round(Julia_total_lnLs1_nFv6, digits=2) == round(Julia_total_lnLs1_GFv6a, digits=2)
@test round(bgb_lnl_nFv6, digits=2) == round(bgb_lnl_GFv6a, digits=2)



print("\nTesting DEC+J traditional SSE likelihood downpass v6 vs. Gflow_arrays v7, with half-matrix:\n")
@test round(Julia_sum_lq_GFv6, digits=1) == round(Julia_sum_lq_GFv6a, digits=1)

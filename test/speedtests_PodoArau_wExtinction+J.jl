...
numareas = 9
n = 2^numareas-1          # 4 areas, 16 states
...
# 2022-03-29: (324.287, 22, -1271.480048120258, -14.85776329108454, -1286.3378114113425, -576.3087517374857)
...
# 2022-03-30: (57.512, 324467, -1271.480048120258, -14.85776329108454, -1286.3378114113425, -576.3087517374857)
...
starttime = Dates.now();
Gflow_to_01_GMRESa  = solve(prob_Gs_v5_sub_i);
endtime = Dates.now();
endtime - starttime  # 11.2 sec

starttime = Dates.now();
Gflow_to_01_GMRESa  = solve(prob_Gs_v5_sub_i, abstol=1e-3);
endtime = Dates.now();
endtime - starttime # 5.753 sec 

starttime = Dates.now();
Gflow_to_01_GMRESb  = solve(prob_Gs_v5_sub_i, abstol=1e-6);
endtime = Dates.now();
endtime - starttime # 5.503 sec

starttime = Dates.now();
Gflow_to_01_GMRESc  = solve(prob_Gs_v5_sub_i, abstol=1e-9);
endtime = Dates.now();
endtime - starttime # 6.114 sec

starttime = Dates.now();
Gflow_to_01_GMRESd  = solve(prob_Gs_v5_sub_i, abstol=1e-12);
endtime = Dates.now();
endtime - starttime # 7.679 sec

starttime = Dates.now();
Gflow_to_01_GMRESe  = solve(prob_Gs_v5_sub_i, abstol=1e-17);
endtime = Dates.now();
endtime - starttime # 7.867 sec
...
endtime = Dates.now();
endtime - starttime # 282 sec
# 2022-03-30 Gmaps.calc_Gs_SSE_sub_i: 336 seconds
# 2022-03-30 Gmaps.calc_Gs_SSE_sub_i: 262 seconds
# 2022-03-30 Gmaps.calc_Gs_SSE_sub_i: 550 seconds -- with Tullio
...
endtime = Dates.now();
endtime - starttime
# 2022-03-30: Gmaps.calc_Gs_SSE: 437 seconds

# "Best case" -- ie matches tradSSE
starttime = Dates.now();
Gflow_to_01_GMRES_res17  = solve(prob_Gs_v5_sub_i, CVODE_BDF(linear_solver=:GMRES), save_everystep=true, abstol=1e-17, reltol=1e-17);
endtime = Dates.now();
endtime - starttime
# 2022-03-31: Gmaps.calc_Gs_SSE_sub_i: 273 seconds

# "Best case" -- ie matches tradSSE
starttime = Dates.now();
Gflow_to_01_GMRES_res18  = solve(prob_Gs_v5_sub_i, CVODE_BDF(linear_solver=:GMRES), save_everystep=true, abstol=1e-18, reltol=1e-18);
endtime = Dates.now();
endtime - starttime
# 2022-03-31: Gmaps.calc_Gs_SSE_sub_i: 273 seconds
...
starttime = Dates.now();
Gflow_to_01_GMRES_res16  = solve(prob_Gs_v5_sub_i, CVODE_BDF(linear_solver=:GMRES), save_everystep=true, abstol=solver_options.abstol, reltol=solver_options.reltol);
endtime = Dates.now();
endtime - starttime
# 2022-03-31: Gmaps.calc_Gs_SSE_sub_i: 154 seconds
...
endtime = Dates.now();
endtime - starttime
# 2022-03-30: Gmaps.calc_Gs_SSE_parallel: 334 seconds
...
# 2022-03-30: (5.779, 22, -1271.7096253140437, -14.759223777102978, -1286.4688490911467, -576.4789837710788)
# 2022-03-30: (18.971, 22, -1271.7096253140437, -14.759223777102978, -1286.4688490911467, -576.4789837710788)
# 2022-03-30: (7.471, 22, -1271.7096253140437, -14.759223777102978, -1286.4688490911467, -576.4789837710788)
# 2022-03-30 Gmaps.calc_Gs_SSE_sub_i: (6.974, 22, -1271.7562065940988, -14.83035764612051, -1286.5865642402193, -576.5877512987153)
# sub_i with broadcast .* etc. is BAD!! (6.72, 22, -1448.9333684287244, -10.498329246124921, -1459.4316976748494, -750.7256784308923)
# 2022-03-31_e16: (7.541, 22, -1270.3548283365074, -228.45071104828136, -1498.8055393847887, -788.9475071715736)

res_Gflow_v6 = iterative_downpass_Gflow_nonparallel_v2!(res; trdf, p_Ds_v5=p_Ds_v5, Gflow=Gflow_to_01_GMRES_res17, solver_options=construct_SolverOpt(), max_iterations=10^10, return_lnLs=true);
(total_calctime_in_sec_GFv6, iteration_number_GFv6, Julia_sum_lq_GFv6, rootstates_lnL_GFv6, Julia_total_lnLs1_GFv6, bgb_lnl_GFv6) = res_Gflow_v6
# (4.145, 22, -1274.8397125331053, -15.30644711896119, -1290.1461596520664, -580.2450686418089)
...
# (9.41, 22, -1248.8727262142502, -16.463790509274336, -1265.3365167235245, -555.6933083815608)
...
# (5.693, 22, -1253.0283992204954, -18.301579784791294, -1271.3299790052868, -561.6183626051958)
...
# (5.838, 22, -1264.8985194593192, -17.145813662231586, -1282.0443331215508, -572.2789957043784)
...
# (6.081, 22, -1277.1175387100548, -14.732718688772852, -1291.8502573988276, -581.8290063335838)
...
# (6.035, 22, -1280.4652766000027, -15.131940509076323, -1295.5972171090789, -585.6942801030481)
...
# (16.268, 22, -1281.7337664124116, -14.34568280229088, -1296.0794492147024, -586.1986751149017)
...
# (8.851, 22, -1274.8397125331053, -15.30644711896119, -1290.1461596520664, -580.2450686418089)
...
starttime = Dates.now();
(Gseg_times, Gflows_array, Gflows_array_totals, Gflows_dict) = Gmap = Gmaps.construct_Gmap_interpolator_float64_parallel(pG, Gseg_times; abstol=solver_options.abstol, reltol=solver_options.reltol);
endtime = Dates.now();
endtime - starttime
...
@test mean(abs.(Gmaps.interp_from_Gmap(root_age, Gmap) .- Gflow_to_01_GMRES(root_age))) < 0.00001
@test mean(abs.(Gmaps.interp_from_Gmap(root_age, Gmap_Double64) .- Gflow_to_01_GMRES(root_age))) < 0.00001

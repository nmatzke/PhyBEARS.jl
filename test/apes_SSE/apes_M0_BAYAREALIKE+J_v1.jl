...
txtvec = ["test: ", "apes_M0_BAYAREALIKE+J_v1.jl", "apes", "areas:2", "states:4", "BAYAREALIKE+J+Yule", "1 like", total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL, R_bgb_lnL]

append_txtvec(results_fn, txtvec)


@testset "Apes BAYAREALIKE+J lnL vs. BioGeoBEARS lnL" begin
...
@testset "Apes BAYAREALIKE+J ancstates vs Julia ancstates (with set parameters)" begin
...
txtvec = ["test: ", "apes_M0_BAYAREALIKE+J_v1.jl", "apes", "areas:2", "states:4", "BAYAREALIKE+J+Yule", "ML", total_calctime_in_sec, iteration_number, Julia_sum_lq, rootstates_lnL, Julia_total_lnLs1, bgb_lnL, R_bgb_lnL]
...
@testset "Apes BAYAREALIKE+J ancstates vs. Julia ancstates (with ML parameters)" begin
	@test all(flat2(compare_dfs(df1bot, df2bot; tol=1e-4) .== 1.0))
	@test all(flat2(compare_dfs(df1top, df2top; tol=1e-4) .== 1.0))
end

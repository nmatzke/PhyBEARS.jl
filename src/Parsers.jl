...
function modify_tiplikes_sampling_fossils_v7!(inputs, p_Ds_v5, geog_df)
	...
	for i in 1:nrow(geog_df)
		...
		if inputs.setup.fossil_TF[nodeNum] == true
			...
			# Rho sampling rates by state; initially fixed to e.g. sampling-prob-at-tip-age-by-state
			# For living tips:
	...
	#inputs.res.normlikes_at_each_nodeIndex_branchTop[nodeNum] .= inputs.res.likes_at_each_nodeIndex_branchTop[nodeNum] ./ sum(inputs.res.likes_at_each_nodeIndex_branchTop[nodeNum])
...
function modify_tiplikes_sampling_fossils_v7old!(inputs, p_Ds_v5, geog_df)
	# Error checks
	taxa = inputs.trdf.taxa
	tipnames = sort(taxa[inputs.trdf.nodeType .== "tip"])
	check_tr_geog_tip_labels(tipnames, geog_df)
...
function juliaRes_to_Rdata(res, trdf, inputs, lnLs_tuple, optim_result, geogfn, trfn; outfns=NaN)
	...
	# BioGeoBEARS_model_object, optim_result, tuple of inputs
	CSV.write(outfns[10], inputs.bmo; delim="\t")
	CSV.write(outfns[11], optim_result; delim="\t")
	tuple_to_Rcode(lnLs_tuple; outfn=outfns[12])
	...
	tuple_to_Rcode(res_inputs_tuple; outfn=outfns[13])
...
build_optim_result(opt, optf, optx, ret; optimfn="optim_result.txt")
"""
function build_optim_result(opt, optf, optx, ret; optimfn="optim_result.txt")
	...
	# Write to file, unless blank
	if (optimfn != "")
		CSV.write(optimfn, optim_results; delim="\t")
	end

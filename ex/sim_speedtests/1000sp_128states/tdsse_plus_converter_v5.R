...
wd = "/Users/nmat471/HD/GitHub/PhyBEARS.jl/ex/sim_speedtests/1000sp_128states/"
setwd(wd)

# Set the random number seed, to make it repeatable
set.seed(543210)  # for 5 tips
set.seed(5423523)		# for 51 tips
...
e_rate = 0.0 # range contraction
j_rate = 0.0 # Not used -- if you want to use it, use in transition_rate_C etc.

dmat = matrix(d_rate, nrow=numstates, ncol=numstates)
elist = rep(e_rate, times=numstates)
amat = matrix(0.0, nrow=numstates, ncol=numstates)


Qmat = rcpp_states_list_to_DEmat(areas_list=area_names, states_list=states_list, dmat=dmat, elist=elist, amat=amat, include_null_range=include_null_range, normalize_TF=TRUE, makeCOO_TF=FALSE)
...
res$inputs$include_null_range = include_null_range
res$outputs = res$inputs$BioGeoBEARS_model_object
res$inputs$max_range_size = numstates
...
transition_matrix_C = get_random_mk_transition_matrix(Nstates=numstates, rate_model="ER", max_rate=0.1)

diag(transition_matrix_C) = 0.0
sum(transition_matrix_C<0)

transition_matrix_C = transition_matrix_C / rowSums(transition_matrix_C)
...
transition_table_C = Carray_df

for (i in 1:nrow(transition_table_C))
	{
	ival = transition_table_C$i[i]
	jval = transition_table_C$j[i]
	kval = transition_table_C$k[i]
	transition_matrix_C[ival,jval] = transition_matrix_C[ival,jval] + transition_table_C$prob[i]/2
	transition_matrix_C[ival,kval] = transition_matrix_C[ival,kval] + transition_table_C$prob[i]/2
	}

rowSums(transition_matrix_C)

TF = rowSums(transition_matrix_C) == 0.0

diag(transition_matrix_C)[TF] = 1.0
rowSums(transition_matrix_C)
...
transition_table_indices_C = as.matrix(transition_table_C[,1:3])

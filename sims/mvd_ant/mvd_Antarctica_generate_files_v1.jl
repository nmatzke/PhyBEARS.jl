...
wd = expanduser("~/GitHub/PhyBEARS.jl/sims/mvd_ant/")
cd(wd)
...
bmo.type[bmo.rownames .== "deathRate"] .= "0.5*birthRate";
bmo.type[bmo.rownames .== "j"] .= "free"
...
# Set up the model
inputs = PhyBEARS.ModelLikes.setup_DEC_SSE2(numareas, tr1, geog_df; root_age_mult=1.01, max_range_size=NaN, include_null_range=include_null_range, bmo=bmo);
(setup, res, trdf, bmo, files, solver_options, p_Ds_v5, Es_tspan) = inputs;

#######################################################
# Read in and parse distances and area-of-areas
#######################################################
files.times_fn = "times_PhyBEARS_1Ma.txt"
files.distances_fn = "distances_5areas.txt"
files.area_of_areas_fn = "areas_5areas.txt" # THIS WAS MISTAKEN 2026-04-02
...
interpolators.times_for_dists_interpolator
interpolators.times_for_SSE_interpolators
interpolators.distances_interpolator(oldest_possible_age)
interpolators.area_of_areas_interpolator(oldest_possible_age)
interpolators.vicariance_mindists_interpolator(oldest_possible_age)
...
Rnames(res)
round.(res.normlikes_at_each_nodeIndex_branchTop[tr1.root]; digits=3)
...
timepoints = sort(unique(vcat(oldest_possible_age, seq(0.0, maximum(geog_interpolator_times), 1.0), geog_interpolator_times)))

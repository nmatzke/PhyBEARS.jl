...
@everywhere include("/GitHub/PhyBEARS.jl/notes/simd_vs_spawn/model_p_object.jl")
...
#include("/GitHub/PhyBEARS.jl/notes/simd_vs_spawn/simd_vs_spawn_setup_v2.jl")
@everywhere include("/GitHub/PhyBEARS.jl/notes/simd_vs_spawn/simd_vs_spawn_setup_v4.jl")
...
@time core_op_simd(u, tspan, p_Ds_v7);
@time core_op_simd_v8(u, tspan, p_Ds_v7);
@time core_op_simd_v8(u, tspan, p_Ds_v7);
...
serial_with_simd_v8(tspan, p_Ds_v7, solve_results1; number_of_solves=number_of_solves)
serial_with_simd_v8(tspan, p_Ds_v7, solve_results1; number_of_solves=number_of_solves)
serial_with_simd_v8(tspan, p_Ds_v7, solve_results1; number_of_solves=number_of_solves)
...
function parallel_with_plain_v7(tspan, p_Ds_v7, solve_results2; number_of_solves=10)
		...
		for j in 1:num_tasks
			if (tasks_fetched_TF[j] == false)
				if (tasks_started_TF[j] == false) && (curr_numthreads > 0)
					# Start a task
					tmpu = deepcopy(solve_results2[j,:])
					push!(tasks, Base.Threads.@spawn core_op_plain(tmpu, tspan, deepcopy(p_Ds_v7)))
			...
			for k in 1:sum(tasks_to_check_TF)
				if (tasks_fetched_TF[current_running_tasks[k]] == false)
					if (istaskstarted(tasks[k]) == true) && (istaskdone(tasks[k]) == true)
						...
						solve_results2[current_running_tasks[k],:] .= sol_Ds_v7.u[length(sol_Ds_v7.u)]
...
parallel_with_plain_v7(tspan, p_Ds_v7, solve_results2; number_of_solves=10)
# (7.535, 8.731365050398926)
# (0.366, 8.731365050398926)
# (0.386, 8.731365050398926)
		...
		for j in 1:num_tasks
			if (tasks_fetched_TF[j] == false)
				if (tasks_started_TF[j] == false) && (curr_numthreads > 0)
					# Start a task
					tmpu = deepcopy(solve_results2[j,:])
					push!(tasks, Base.Threads.@spawn core_op_simd(tmpu, tspan, deepcopy(p_Ds_v7)))
					curr_numthreads = curr_numthreads-1;
					tasks_started_TF[j] = true;
					push!(current_running_tasks, task_numbers[j])
				end
...
function parallel_with_simd_v8(tspan, p_Ds_v7, solve_results2; number_of_solves=10)
	start_time = Dates.now()
	number_of_threads = Base.Threads.nthreads()
	curr_numthreads = Base.Threads.nthreads()
		...
		for j in 1:num_tasks
			if (tasks_fetched_TF[j] == false)
				if (tasks_started_TF[j] == false) && (curr_numthreads > 0)
					# Start a task
					tmpu = deepcopy(solve_results2[j,:])
					push!(tasks, Base.Threads.@spawn core_op_simd_v8(tmpu, tspan, deepcopy(p_Ds_v7)))
					curr_numthreads = curr_numthreads-1;
					tasks_started_TF[j] = true;
					push!(current_running_tasks, task_numbers[j])
				end
...
tspan = (0.0, 1.0)
parallel_with_plain_v7(tspan, p_Ds_v7, solve_results2; number_of_solves=number_of_solves)
...
# Term floats are included inside function
parallel_with_simd_v8(tspan, p_Ds_v7, solve_results2; number_of_solves=number_of_solves)

...
function core_op_plain(u, tspan, p_Ds_v7)
	...
	sol_Ds_v5 = solve(prob_Ds_v5, Tsit5(), save_everystep=false, abstol=1e-12, reltol=1e-9);
	return sol_Ds_v5
end
...
function parallel_with_plain_v5(tspan, p_Ds_v7, solve_results2; number_of_solves=10)
	...
	if (num_workers != (numthreads-1))
		txt = join(["STOP ERROR in parallel_with_plain_v5(): You have Distributed.workers()=", string(num_workers), " workers/processors enabled with Distributed.addprocs(), so you should have had ", string(num_workers+1), " threads available. Instead Threads.nthreads()=", string(Threads.nthreads()), ". Re-start Julia with e.g. JULIA_NUM_THREADS=", string(num_workers+1), " julia to fix this. Also make sure to use Distributed.addprocs to add N-1 processors."])
		print("\n")
		print(txt)
		print("\n")
		error(txt)

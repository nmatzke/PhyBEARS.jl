...
# nprocs
"""
function iterative_downpass_parallel_ClaSSE_v7d!(res; trdf, p_Ds_v7, solver_options=construct_SolverOpt(), max_iterations=10^10, return_lnLs=false, include_null_range=true)
...
function iterative_downpass_parallel_ClaSSE_v7e!(res; trdf, p_Ds_v7, solver_options=construct_SolverOpt(), max_iterations=10^10, return_lnLs=false, include_null_range=true)
	diagnostics = collect(repeat([Dates.now()], 3))
	diagnostics[1] = Dates.now()
...
# 					else
					...
					spawned_nodeIndex = spawned_nodeIndices[manual_iter]

...
p_inputsG = (n=p_Ds_v5.n, params=p_Ds_v5.params, p_indices=p_Ds_v5.p_indices, p_TFs=p_Ds_v5.p_TFs, uD=uD, uE=p_Ds_v5.uE, sol_Es_v5=sol_Es_v5);
...
############################################################
# Gflow calculated in steps of an array
############################################################
# This works OK at time_increment=0.01, but only has half of the multiplications, 2nd one uses Gflow(tspan[2])
"""
include("/GitHub/PhyBEARS.jl/notes/Flow.jl"); import .Flow
"""
Gseg_times = seq(0.0, 2.5, 0.001);  # Times for Gsegments
pG = (n=n, p_Ds_v5=p_inputsG, A=A);
(Gflows_array, Gflows_array_totals) = Flow.calc_Gmaps_SSE(pG, Gseg_times; abstol=1e-12, reltol=1e-12);
...
# abstol=1e-9:  (0.208, 2, -5.917627381773604, -2.882205463015512, -8.799832844789115, -3.9520899056719028)
# abstol=1e-12: (0.212, 2, -5.917628361171168, -2.8822052907290074, -8.799833651900176, -3.9520907740621976)

# ...but seems to break down with fewer increments, even with tighter abstol...
...
# abstol=1e-9: (0.002, 2, -6.1552282789845005, -2.882205305194796, -9.037433584179297, -4.189690701196094)
# abstol=1e-12: (0.003, 2, -6.155228351794358, -2.8822052925834836, -9.037433644377842, -4.189690765880259)
############################################################

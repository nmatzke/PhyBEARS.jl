...
include("/GitHub/PhyBEARS.jl/notes/timed_runs_v2_smooth_interpolation.jl")
"""
...
timed_run_GflowArrays_CVODE(trfn, geogfn; bmo=NaN, include_null_range=false, tol=1e-3, Gseg_times=NaN, num_incs=NaN)
timed_run_GflowArrays_CVODE_dbl(trfn, geogfn; bmo=NaN, include_null_range=false, tol=1e-3, Gseg_times=NaN, num_incs=NaN)
...
i+=1; tres[i]=timed_run_GflowArrays_CVODE(trfn, geogfn; bmo=NaN, include_null_range=false, tol=1e-12, Gseg_times=NaN, num_incs=100)
...
tdf1 = indexed_Dict_to_DF(tres)
df_to_Rdata(tdf1; fn="tdf1a_smooth.RData")
...
tdf2 = indexed_Dict_to_DF(tres)
df_to_Rdata(tdf2; fn="tdf2a_smooth.RData")
...
df_to_Rdata(tdf3; fn="tdf3a_smooth.RData")
...
tdf1 = indexed_Dict_to_DF(tres)
CSV.write("tdf1b_cont_smooth.RData", tdf1, delim="\t")
...
tdf2 = indexed_Dict_to_DF(tres)
CSV.write("tdf2b_cont_smooth.RData", tdf2, delim="\t")
...
tdf3 = indexed_Dict_to_DF(tres)
CSV.write("tdf3b_cont_smooth.RData", tdf3, delim="\t")
...
function timed_run_goldstandard(trfn, geogfn; bmo=NaN, include_null_range=false, tol=1e-6)
	...
	sol_Es_v5 = solve(prob_Es_v5, solver_options.solver, save_at=solver_options.saveat, save_everystep=true, abstol=solver_options.abstol, reltol=solver_options.reltol);
	push!(calctimes, (Dates.now() - tmpt).value/1000.0);
...
function timed_run_trad_solverFree(trfn, geogfn; bmo=NaN, include_null_range=false, tol=1e-6)
	...
	# The Es solver must be continuous!
	prob_Es_v5 = DifferentialEquations.ODEProblem(parameterized_ClaSSE_Es_v5, p_Ds_v5.uE, Es_tspan, p_Ds_v5);
	sol_Es_v5 = solve(prob_Es_v5,  save_at=solver_options.saveat, save_everystep=true, abstol=solver_options.abstol, reltol=solver_options.reltol);
	push!(calctimes, (Dates.now() - tmpt).value/1000.0);
...
function timed_run_Gflow_CVODE(trfn, geogfn; bmo=NaN, include_null_range=false, tol=1e-6)
	...
	#Gflow_to_01_GMRES  = solve(prob_Gs_v5_sub_i, solver_options.solver, save_at=solver_options.saveat, save_everystep=solver_options.save_everystep, abstol=solver_options.abstol, reltol=solver_options.reltol, dense=true);
	Gflow_to_01_GMRES  = solve(prob_Gs_v5_sub_i, save_at=solver_options.saveat, save_everystep=solver_options.save_everystep, abstol=solver_options.abstol, reltol=solver_options.reltol);
...
function timed_run_GflowArrays_CVODE(trfn, geogfn; bmo=NaN, include_null_range=false, tol=1e-6, Gseg_times=NaN, num_incs=NaN)
	...
	prob_Gs_v5_sub_i = DifferentialEquations.ODEProblem(Gmaps.calc_Gs_SSE_sub_i, G0, tspan, pG);
	#Gflow_to_01_GMRES  = solve(prob_Gs_v5_sub_i, solver_options.solver, save_at=solver_options.saveat, save_everystep=solver_options.save_everystep, abstol=solver_options.abstol, reltol=solver_options.reltol, dense=true);
	#Gflow_to_01_GMRES  = solve(prob_Gs_v5_sub_i, save_at=solver_options.saveat, save_everystep=solver_options.save_everystep, abstol=solver_options.abstol, reltol=solver_options.reltol, dense=true);
...
(total_calctime_in_sec_GFv6, iteration_number_GFv6, Julia_sum_lq_GFv6, rootstates_lnL_GFv6, Julia_total_lnLs1_GFv6, bgb_lnl_GFv6) = res_Gflow_v6
	...
	return(df)
end # END timed_run_GflowArrays_CVODE_dbl

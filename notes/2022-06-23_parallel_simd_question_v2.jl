...
# Inner-loop functions for solving "Ds" -- solve() has to be run many times
function sum_Crates_Ds_simd(Cijk_rates_sub_i, tmp_u, tmp_uE, Cj_sub_i, Ck_sub_i)
    term1 = 0.0; term4 = 0.0
...
function sum_Qrates_simd(Qij_vals_sub_i, tmp_u, Qj_sub_i)
    term2 = 0.0; term3 = 0.0
...
ODE_to_solve_Ds_simd = (du,u,p,t) -> begin
  n = p.n
  mu = p.params.mu_vals
	uE = p.sol_Es_v5(t)
	terms = p.terms
  for i in 1:n
		terms .= 0.0
		terms[1], terms[4] = sum_Crates_Ds_simd(p.p_TFs.Cijk_rates_sub_i[i], u, uE, p.p_TFs.Cj_sub_i[i], p.p_TFs.Ck_sub_i[i])
		terms[2], terms[3] = sum_Qrates_simd(p.p_TFs.Qij_vals_sub_i[i], u, p.p_TFs.Qj_sub_i[i])
...
# ODEs used to solve "Es" -- solve() only runs once
ODE_to_solve_Es_simd = (du,u,p,t) -> begin
  n = p.n
  mu = p.params.mu_vals
	terms = p.terms
  for i in 1:n
		terms .= 0.0
		terms[1], terms[4] = sum_Crates_Es_simd(p.p_TFs.Cijk_rates_sub_i[i], u, p.p_TFs.Cj_sub_i[i], p.p_TFs.Ck_sub_i[i])
		terms[2], terms[3] = sum_Qrates_simd(p.p_TFs.Qij_vals_sub_i[i], u, p.p_TFs.Qj_sub_i[i])
		du[i] = mu[i] -(terms[1] + terms[2] + mu[i])*u[i] + terms[3] + terms[4]
  end
end

# Inner loop for Es -- used only in ODE_to_solve_Es_simd()
function sum_Crates_Es_simd(Cijk_rates_sub_i, tmp_u, Cj_sub_i, Ck_sub_i)
    term1 = 0.0; term4 = 0.0
    @inbounds @simd for it=1:length(Cijk_rates_sub_i)
    	term1 += Cijk_rates_sub_i[it]
...
# This is the core operation to solve "Ds"; @simd version
function core_op_simd(u, tspan, p_Ds_v7)
	prob_Ds_v7 = DifferentialEquations.ODEProblem(ODE_to_solve_Ds_simd, u.+0.0, tspan, p_Ds_v7);
...
function serial_solves_simd(tspan, p_Ds_v7, solve_results1; number_of_solves=8)
	start_time = Dates.now()
	for i in 1:number_of_solves
		solve_results1[i,:] .= 0.0
		solve_results1[i,i] = 1.0
...
function parallel_solves_simd(tspan, p_Ds_v7, solve_results2; number_of_solves=8)
	print("\nSolved task #: ")
	start_time = Dates.now()
	# Make tasks, then run them
	tasks = Any[]
	...
	for i in 1:number_of_solves
		solve_results2[i,i] = 1.0 
		# CHANGED TO SIMD HERE
		push!(tasks, Base.Threads.@spawn core_op_simd(solve_results2[i,:].+0.0, tspan, p_Ds_v7));
		push!(tasks_fetched_TF, false)
	end
...
# Precalculate the ODE Es for use in the ODE Ds
Es_tspan = (0.0, 2.0)
prob_Es_v7 = DifferentialEquations.ODEProblem(ODE_to_solve_Es_simd, p_Es_v5.uE, Es_tspan, p_Es_v5);
...
serial_solves_simd(tspan, p_Ds_v7, solve_results1; number_of_solves=number_of_solves)
serial_solves_simd(tspan, p_Ds_v7, solve_results1; number_of_solves=number_of_solves)
serial_solves_simd(tspan, p_Ds_v7, solve_results1; number_of_solves=number_of_solves)

# Multiple ODE solves, parallel versions
parallel_solves_plain(tspan, p_Ds_v7, solve_results2; number_of_solves=8)
parallel_solves_plain(tspan, p_Ds_v7, solve_results2; number_of_solves=8)
parallel_solves_plain(tspan, p_Ds_v7, solve_results2; number_of_solves=8)

# Usually hangs
parallel_solves_simd(tspan, p_Ds_v7, solve_results2; number_of_solves=number_of_solves)

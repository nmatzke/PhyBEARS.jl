...
"""
function iterative_downpass_nonparallel_ClaSSE_v6_solverFree!(res; trdf, p_Ds_v5, solver_options=construct_SolverOpt(), max_iterations=10^10, return_lnLs=false, include_null_range=true)
...
# 					print("\n\n12345\n\n")
	...
	solvers = collect(repeat([Any[]]))
	for i in 1:length(tasks)
		solvers[i] = tasks[i].alg
	end
	solvers_count_dict = StatsBase.countmap(a; alg = :auto)
	maxTF = solvers_count_dict.vals .== maximum(solvers_count_dict.vals)
	most_common_solver = solvers_count_dict.keys[maxTF][1]
	...
	if return_lnLs == true
		...
		return(total_calctime_in_sec, iteration_number, most_common_solver)
	end

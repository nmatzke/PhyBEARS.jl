...
using BioGeoJulia.TreeTable  # for get_nonrootnodes_trdf
...
export parameterized_ClaSSE_As_v7, parameterized_ClaSSE_As_v6, parameterized_ClaSSE_As_v5, check_linearDynamics_of_As, calc_Gs_SSE_condnums!, calc_Gs_SSE, calc_Gs_SSE_v7!, calc_Gmaps_SSE, calc_Gs_SSE!, branchOp_ClaSSE_Gs_v5, iterative_downpass_nonparallel_FlowClaSSE_v5!, iterative_downpass_Gflow_nonparallel_v3, run_Gs



# Trying this again, including the Ds as well as Es as inputs
...
# calculation of A
parameterized_ClaSSE_As_v7 = (t, A, p; max_condition_number=1e8, print_warnings=true) -> begin
...
#			print(typeof(uD))
#			print("\n")
#			print(typeof(uD[Cj_sub_i[m]]))
			#
			#rate_sp_then_ex1 = Cijk_vals[Ci_eq_i][m] * normalized_uD[Cj_sub_i[m]] * uE[Ck_sub_i[m]] # rate of going to j, times k going extinct
			#rate_sp_then_ex2 = Cijk_vals[Ci_eq_i][m] * normalized_uD[Ck_sub_i[m]] * uE[Cj_sub_i[m]] # rate of going to k, times j going extinct
...
# This version excludes Xi (and Xj), just like castor's get_LinearDynamics_A
# calculation of A
parameterized_ClaSSE_As_v6 = (t, A, p; max_condition_number=1e8, print_warnings=true) -> begin
  ...
  @inbounds for i in 1:n
		...
		@inbounds for m in 1:length(Ci_sub_i)
			...
			rate_sp_then_ex1 = Cijk_vals[Ci_eq_i][m] * uE[Ck_sub_i[m]] # rate of going to j, times k going extinct
			rate_sp_then_ex2 = Cijk_vals[Ci_eq_i][m] * uE[Cj_sub_i[m]] # rate of going to k, times j going extinct
			A[Ci_sub_i[m],Cj_sub_i[m]] = A[Ci_sub_i[m],Cj_sub_i[m]] + rate_sp_then_ex1^2
			A[Ci_sub_i[m],Ck_sub_i[m]] = A[Ci_sub_i[m],Ck_sub_i[m]] + rate_sp_then_ex2^2
...
Gflows_array = calc_Gmaps_SSE(pG, Gseg_times)
"""
function calc_Gmaps_SSE(pG, Gseg_times)
	total_tspan = (minimum(Gseg_times), maximum(Gseg_times));
	num_segments = length(Gseg_times)-1
	Gflows_array = zeros(Float64,pG.n,pG.n,num_segments)
	Gflow_total = Matrix{Float64}(I, pG.n, pG.n);
	
	for inc in 1:num_segments
		Gflow_total_old = deepcopy(Gflow_total)
		G0 = Matrix{Float64}(I, pG.n, pG.n) ;
		# build an A (the linear dynamics, i.e. Q and C matrices combined into a square matrix)
		pG.A[:,:] .= 0.0
		tspan = (Gseg_times[inc], Gseg_times[inc+1])
		...
		Gres = Gflow_to_01_GMRES(round(Gseg_times[inc+1], digits=5))
...
end # END function calc_Gmaps_SSE(pG, times)
...
Non-parallel version (no istaskdone, etc.)
"""
function iterative_downpass_Gflow_nonparallel_v3!(res; trdf, p_Ds_v5, Gflow, Gseg_times, Gflows_array, solver_options=construct_SolverOpt(), max_iterations=10^10, return_lnLs=false, include_null_range=true)
	#######################################################
...
#				push!(tasks, @spawn branchOp(current_nodeIndex, res, num_iterations=num_iterations))
#			else
			tmp_results = branchOp_ClaSSE_Gflow_v2(current_nodeIndex, res, Gflow, Gseg_times, Gflows_array, tspan=tspan, p_Ds_v5=p_Ds_v5, solver_options=solver_options)
			#tmp_results = branchOp(current_nodeIndex, res, num_iterations)
...
# 					print("\n\n12345\n\n")
					...
					if (sum(edge_rows_TF) > 3)
						txt = "STOP error in: iterative_downpass_Gflow_nonparallel_v3() - node has more than 2 edges"
					end
	...
	# shouldn't get here
	return NaN
end # END iterative_downpass_Gflow_nonparallel_v3!
...
# it created conflicts apparently when there were more @spawns than cores
# Do all the writing to res in the while() loop
function branchOp_ClaSSE_Gflow_v2(current_nodeIndex, res, Gflow, Gseg_times, Gflows_array; tspan, p_Ds_v5, solver_options=solver_options)
	calc_start_time = Dates.now()
	spawned_nodeIndex = current_nodeIndex
	tmp_threadID = Threads.threadid()
...
#		fakeX0 = factorize(Gflow_to_current_branchtop) \ tmpX0s
	...
	#for index in (start_seg+1):end_seg 
		#mul!(Xparent_tmp, Gflows_array[:,:,index], Xparent_tmp[:])
		#Xparent_tmp = Gflows_array[:,:,index] * Xparent_tmp[:]
	#end
...
end # END branchOp_ClaSSE_Gflow_v2

...
# 2022-04-19_NJM: adds a transition_table_indices_C, representing the joint probabilities of cladogenetics events; as in simulate_tdsse(), 
# Also adds transition_table_probs_C, size num_clado_events*NT, representing a changing transition probabilities t different timesteps
# The simulation is halted as soon as Ntips>=max_tips (if max_tips>0) and/or time>=max_time (if max_time>0)
simulate_tdsse2 = function(	Nstates,							# number of discrete possible states for the trait
	...
	if(is.null(max_tips) && is.null(max_time)) stop("ERROR in simulate_tdsse2(): At least one of max_tips and/or max_time must be non-NULL")
	is_hisse_model = !(is.null(NPstates) || (NPstates==0) || (NPstates==Nstates))
	if(is_hisse_model && is.null(proxy_map)) stop("ERROR in simulate_tdsse2(): Missing proxy_map, needed for HiSSE model")
	if(is_hisse_model && (length(proxy_map)!=Nstates)) stop("ERROR in simulate_tdsse2(): proxy_map has length %d, but should have length %d (Nstates)",length(proxy_map),Nstates)
	if(is_hisse_model && (length(unique(proxy_map))!=NPstates)) stop("ERROR in simulate_tdsse2(): Not all %d proxy states are represented in proxy_map",NPstates)
	if((!is_hisse_model) && (!is.null(proxy_map)) & ((length(proxy_map)!=Nstates) || (any(proxy_map!=(1:Nstates))))) stop("ERROR in simulate_tdsse2(): Non-trivial proxy_map contradicts non-HiSSE model")
	if(!is_hisse_model) NPstates = Nstates
	if(is.null(time_grid)) stop("ERROR in simulate_tdsse2(): Missing time_grid")
	...
	parameter_names = c("transition_matrix_A", "transition_matrix_C", "transition_table_indices_C", "transition_table_probs_C", "birth_rates", "death_rates")
	...
	if(is.null(parameters$birth_rates)){
		...
		stop("ERROR in simulate_tdsse2(): birth_rates must either be empty, a single number or a 2D matrix")
	}else if(nrow(parameters$birth_rates)!=Nstates){
		stop(sprintf("ERROR in simulate_tdsse2(): Invalid number of rows in birth_rates; expected %d (Nstates), but got %d",Nstates,nrow(parameters$birth_rates)))
	}else if(ncol(parameters$birth_rates)!=NG){
		stop(sprintf("ERROR in simulate_tdsse2(): Invalid number of columns in birth_rates; expected %d (Ngrid), but got %d",NG,ncol(parameters$birth_rates)))
	}
	...
	if(is.null(parameters$death_rates)){
		...
		stop("ERROR in simulate_tdsse2(): death_rates must either be empty, a single number or a 2D matrix")
	}else if(nrow(parameters$death_rates)!=Nstates){
		stop(sprintf("ERROR in simulate_tdsse2(): Invalid number of rows in death_rates; expected %d (Nstates), but got %d",Nstates,nrow(parameters$death_rates)))
	}else if(ncol(parameters$death_rates)!=NG){
		stop(sprintf("ERROR in simulate_tdsse2(): Invalid number of columns in death_rates; expected %d (Ngrid), but got %d",NG,ncol(parameters$death_rates)))
	}
	# start state
	if(is.null(start_state)){
		start_state = sample.int(n=Nstates,size=1)
	}else if(!(start_state %in% (1:Nstates))){
		stop(sprintf("ERROR in simulate_tdsse2(): Invalid start_state (%d): Must be an integer between 1 and %d",start_state,Nstates))
	}
	# sampling fractions
	if(is.null(sampling_fractions) || (length(sampling_fractions)==0)){
		...
		stop(sprintf("ERROR in simulate_tdsse2(): Invalid number of sampling fractions (%d), expected either 0, 1 or %d (NPstates)",length(sampling_fractions),NPstates))
	}
	...
	if(is.null(reveal_fractions) || (length(reveal_fractions)==0)){
		...
		stop(sprintf("ERROR in simulate_tdsse2(): Invalid number of reveal fractions (%d), expected either 0, 1 or %d (NPstates)",length(reveal_fractions),NPstates))
	}
	# anagenetic transition rates
	if(!is.null(parameters$transition_matrix_A)){
		if((!is.array(parameters$transition_matrix_A)) && (!is.matrix(parameters$transition_matrix_A))) stop("ERROR in simulate_tdsse2(): transition_matrix_A must be a 2D matrix or a 3D array")
		if(nrow(parameters$transition_matrix_A)!=Nstates) stop(sprintf("ERROR in simulate_tdsse2(): transition_matrix_A must have %d (Nstates) rows, but instead found %d",Nstates,nrow(parameters$transition_matrix_A)))
		if(ncol(parameters$transition_matrix_A)!=Nstates) stop(sprintf("ERROR in simulate_tdsse2(): transition_matrix_A must have %d (Nstates) columns, but instead found %d",Nstates,ncol(parameters$transition_matrix_A)))
		if(is.matrix(parameters$transition_matrix_A)){
			...
			stop(sprintf("ERROR in simulate_tdsse2(): transition_matrix_A must have %d layers (Ngrid), but instead found %d",NG,dim(parameters$transition_matrix_A)[3]))
		}
		if(any(abs(rowSums(parameters$transition_matrix_A[,,1]))>1e-6*max(abs(parameters$transition_matrix_A[,,1])))) stop("ERROR in simulate_tdsse2(): Anagenetic transition rate matrix does not seem to be valid; at time %g some row sums are not zero.",time_grid[1])
	}
	# cladogenic transition rates
	if(!is.null(parameters$transition_matrix_C)){
		if((!is.array(parameters$transition_matrix_C)) && (!is.matrix(parameters$transition_matrix_C))) stop("ERROR in simulate_tdsse2(): transition_matrix_C must be a 2D matrix or a 3D array")
		if(nrow(parameters$transition_matrix_C)!=Nstates) stop(sprintf("ERROR in simulate_tdsse2(): transition_matrix_C must have %d (Nstates) rows, but instead found %d",Nstates,nrow(parameters$transition_matrix_C)))
		if(ncol(parameters$transition_matrix_C)!=Nstates) stop(sprintf("ERROR in simulate_tdsse2(): transition_matrix_C must have %d (Nstates) columns, but instead found %d",Nstates,ncol(parameters$transition_matrix_C)))
		if(is.matrix(parameters$transition_matrix_C)){
			...
			stop(sprintf("ERROR in simulate_tdsse2(): transition_matrix_C must have %d layers (Ngrid), but instead found %d",NG,dim(parameters$transition_matrix_C)[3]))
		}
		if(any(parameters$transition_matrix_C<0)) stop("ERROR in simulate_tdsse2(): Cladogenic transition probability matrix does not seem to be valid; some entries are negative.")
		if(any(abs(rowSums(parameters$transition_matrix_C[,,1])-1)>1e-6)) stop("ERROR in simulate_tdsse2(): Cladogenic transition probability matrix does not seem to be valid; at time %g some row sums differ from 1.",time_grid[1])
	} # END if(!is.null(parameters$transition_matrix_C))
	
	# 2022-04-19_NJM_added:
	# cladogenic transition rates -- as a table, or, in this case, an array representing the cladogenetic table through time
	if(!is.null(parameters$transition_table_indices_C)){
		if( (!is.matrix(parameters$transition_table_indices_C))) stop("ERROR in simulate_tdsse2(): transition_table_indices_C must be a 2D matrix")
		if(nrow(parameters$transition_table_indices_C)!=nrow(parameters$transition_table_probs_C)) stop(sprintf("ERROR in simulate_tdsse2(): nrow(transition_table_indices_C)=%d must equal nrow(parameters$transition_table_probs_C)= %d, but instead they disagree.",nrow(parameters$transition_table_indices_C),nrow(parameters$transition_table_probs_C)))
		if(ncol(parameters$transition_table_indices_C)!=3) stop(sprintf("ERROR in simulate_tdsse2(): transition_table_indices_C must have 3 columns, for i,j,k, but instead found %d",ncol(parameters$transition_table_indices_C)))

		if(any(parameters$transition_table_probs_C<0)) stop("ERROR in simulate_tdsse2(): Cladogenic transition table probabilities (transition_table_probs_C) does not seem to be valid; some entries are negative.")
	} # END if(!is.null(parameters$transition_table_C))
	
	# sampling fractions
	if(any(sampling_fractions<=0 | sampling_fractions>1)) stop("ERROR in simulate_tdsse2(): sampling_fractions must be between 0 (non-inclusive) and 1 (inclusive).")

	# check if some passed parameters are not recognized
	invalids = setdiff(names(parameters),parameter_names)
	if(length(invalids)>0) stop(sprintf("ERROR in simulate_tdsse2(): Unknown parameter '%s'",invalids[1]))

	results = generate_random_tree_tdSSE_CPP2(	max_tips					= (if(is.null(max_tips)) -1 else max_tips),
												...
												transition_table_indices_C			= as.vector(t(parameters$transition_table_indices_C)), # 0-based integers, columns i,j,k; flatten in row-major format
												transition_table_probs_C			= (if(is.null(parameters$transition_table_probs_C)) numeric() else as.vector(aperm(parameters$transition_table_probs_C,c(3,2,1)))), # flatten in layer-row-major format

...
simulate_dsse2 = function(	Nstates,							# number of discrete possible states for the trait
	...
	#print(parameters$transition_table_indices_C)
	if(is.null(parameters$transition_table_indices_C)) stop("STOP ERROR: simulate_dsse2() *requires* parameters$transition_table_indices_C!");
	#print(parameters$transition_table_probs_C)
	if(is.null(parameters$transition_table_probs_C)) stop("STOP ERROR: simulate_dsse2() *requires* parameters$transition_table_probs_C!");
	...
	if(any(abs(rowSums(parameters$transition_matrix_C)-1)>1e-6)) stop("STOP ERROR in simulate_dsse2(): Cladogenic transition probability matrix does not seem to be valid; some row sums differ from 1.")
	...
	results = generate_random_tree_Mk_rates_CPP2(max_tips					= (if(is.null(max_tips)) -1 else max_tips),
												...
												transition_table_probs_C			= as.vector(t(parameters$transition_table_probs_C)), # vector of floats

...
# NJM: Setting the anagenetic rates of A->null and B-> to zero
A[,1] = 0.0
...
# NJM: Setting extinction to 0.0
# NJM: Setting birth rate to a low value, so that the anagenetic transitions
#      "A" will dominate - i.e., they are highly likely to happen before the 
#      first speciation event in the simulation (which becomes the root)
parameters = list(birth_rates         = 0.0001,
                  death_rates         = 0.0,
...
# Debug
NPstates = NULL; proxy_map = NULL; 
    splines_degree = 1; start_state = NULL; 
    max_tips = NULL; max_time = NULL; max_events = NULL; sampling_fractions = NULL; 
    reveal_fractions = NULL; coalescent = TRUE; as_generations = FALSE; 
    no_full_extinction = TRUE; Nsplits = 2; tip_basename = ""; 
    node_basename = NULL; include_birth_times = FALSE; include_death_times = FALSE; 
    include_labels = TRUE
Nstates = 4; 
                            parameters = parameters; 
														splines_degree      = 1;
...
print(ind)
print(simstates)
print(simstates[match_simtree_in_new1])
print(simstates[match_simtree_in_new2])
#}